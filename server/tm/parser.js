...
const getTeamName = async (name) => {
  const list = await teamnameSchema
    .find({ soccerway: new RegExp(name, "i") })
    .exec();
...
const getTeamByName = async (teamName) =>
  await teamslistSchema.findOne({ name: new RegExp(teamName, "i") }).exec();

const checklist = async (teamname) =>
  await teamnameSchema.findOne({ soccerway: new RegExp(teamName, "i") });
...
const addTM = async (TM) => {
  try {
    const { soccerway: name, tm_link: URL } = TM;
...
const searchTeam = async (team) => {
  const getingTeamsAndSquads = async (teamname) => {
    const find = await teamslistSchema
      .findOne({ name: new RegExp(teamname, "ig") })
      .exec();
              ...
              $(search_list).each(async (index, item) => {
                ...
                if (name === teamname) {
                  ...
                  const href = $(item)
                    ...
                    .attr("href");
                  const name = teamname;
                  const squad_list = await getSquad({ href });
                  if (
                    !isEmptyObject(href) &&
                    !isEmptyObject(name) &&
                    !isEmptyObject(total_market_value) &&
  ...
  try {
    ...
    const AddNames = async (Names) =>
      await new Promise(async (resole) => {
        ...
        for (const line of Names) {
          const teamname = new teamnameSchema({ ...line });
    ...
    const AddList = (lists) => {
      for (const line of lists) {
        const teamname = new teamslistSchema({ ...line });
        teamname.save();
      }
      return true;
    };
...
const updateTeamById = async (id) => {
  try {
    ...
    await teamslistSchema.deleteOne({ name: updateLinks.soccerway }).exec();
