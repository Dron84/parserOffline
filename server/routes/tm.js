const express = require("express"),
  ...
  { isEmptyObject, setLog } = require("../services/functions");
...
tm.get("/clear", async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Transfermarket is cleared" });
  } catch (e) {
...
tm.get("/delete", async (req, res) => {
  try {
    ...
    res.status(200).json({
      backup: "file",
      result: { teamname: result_teamname, teamlist: result_teamlist },
    });
  } catch (e) {
