...
const store = new Vuex.Store({
  ...
  actions: {
    ...
    async CHECK_PRICE(state, teamName) {
      const { data } = await $axios.get(`tm/checkname/${encodeURI(teamName)}`);
      return data;
    },
    async ADD_TM_NAME(state, obj) {
      const { data } = await $axios.post(`tm/addtmname`, obj);
      return data;
    },
    async PARSE_URL(state, obj) {
      state.commit("SET_PRELOADER", true);
      if (obj.team === "match") {
        const { data } = await $axios.post("parse", { url: obj.url });
        data.teamA.comment = "";
        data.teamA.addsquad = [];
        data.teamB.comment = "";
        data.teamB.addsquad = [];
        ...
        data.addsquad = [];
        state.commit("SET_PRELOADER", false);
        state.commit("SET_TEAM", { team, data });
        return data;
      }
    },
    ...
    // Если заказчик передумает сохранять данные локально
    // async SAVE_STATUS_TO_SERVER(state, obj) {
    //   const { teamsId, id, item_id } = obj;
    //   return await $axios.post(`gamerstatus/${teamsId}/${id}/${item_id}`);
    // },
    async SAVE_STATUS(state, obj) {
      const { teamsId, id, item_id, name } = obj;
    ...
    //  Если заказчик передумает сохранять данные локально
    // async SAVE_LINEUP_TO_SERVER(state, obj) {
    //   const { shirtnumber, teamsId, lineupStatus } = obj;
    //   return await $axios.post(
    //     `lineupstatus/${teamsId}/${shirtnumber}/${lineupStatus}`
    //   );
    // },
    async SAVE_LINEUP(state, obj) {
      const { shirtnumber, teamsId, lineupStatus, name } = obj;
      let status;
    ...
    async SAVE_NEW_MATCH(state, obj) {
      const { shirtnumber, teamsId, matchStatus, name } = obj;
      let status;
      if (matchStatus === null) {
        status = "squad";
